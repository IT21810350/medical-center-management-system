const Equipment = require('../Models/LabAssistantModel/equipmentModel');

// EQUIPMENT
// Equipment Controllers for each CRUD function
module.exports.createEquipments = async (req, res, next) => {
  try {
    const { equipment_name, manufacturer, purchase_date, maintainence_schedule, status } = req.body;
...
module.exports.deleteEquipmentById = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Equipment with ID: " + equipmentId + " has been deleted" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
  }
