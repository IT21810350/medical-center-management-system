...
function LabTestPage() {
  ...
  const handleCreateResult = () => {
    const newResult = {
      ...
      test_date: new Date(newResultTestDate),
            ...
            <TableBody>
              {labTestResults.map((result) => (
                <TableRow key={result.result_id}>
                  ...
                  <TableCell>{result.test_date.toISOString().slice(0, 10)}</TableCell>
      ...
      <Dialog open={openResultDialog} onClose={() => setOpenResultDialog(false)}>
        ...
        {selectedResult && (
          <DialogContent>
            ...
            <Typography>Test Date: {selectedResult.test_date.toISOString().slice(0, 10)}</Typography>
